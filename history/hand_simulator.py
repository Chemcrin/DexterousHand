...
class HandModel:
    ...
    def __init__(self):
        ...
        self.finger_base_offsets = {
            'thumb': np.array([-2.5, -1.0, 0.0]),
            'index': np.array([-1.5, 0.0, 0.0]),
            'middle': np.array([0.0, 0.5, 0.0]),
            'ring': np.array([1.5, 0.0, 0.0]),
            'pinky': np.array([2.8, -0.8, 0.0])
        }
        self._build_thumb()
    ...
    def _build_thumb(self):
        ...
        chain.add_joint(self.finger_base_offsets['thumb'], axes_cmc)
        chain.add_joint(np.array([0, l0, 0]), axes_mcp)
    ...
    def apply_gesture(self, gesture_name: str):
        ...
        if gesture_name == 'thumbs_up':
            ...
            self.set_joint_angle('thumb', 0, 1, np.radians(30))
...
def setup_interactive_controls(hand_model: HandModel, fig: plt.Figure, 
    ...
    n_cols, slider_height, slider_spacing = 4, 0.03, 0.01
    bottom_start, slider_width, left_margin = 0.25, 0.15, 0.1
    ...
    for i, (finger, joint_idx, dof_idx, angle_rad, _) in enumerate(controller.joint_params):
        ...
        slider = Slider(ax_slider, label, -90.0, 90.0, valinit=np.degrees(angle_rad), valstep=1.0)
    ...
    for i, gesture in enumerate(gestures):
        ...
        # 关键修正：使用默认参数捕获当前 gesture
        def callback(event, g_name=gesture):
            if g_name == 'reset':
