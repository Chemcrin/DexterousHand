...
# Matplotlib's default DejaVu Sans font does not contain CJK glyphs. If a
# Chinese-capable font is available, the animation title keeps the Chinese pose
# name; otherwise the title falls back to English to avoid tofu boxes such as □□□.
CJK_SAMPLE_TEXT = "摊开手掌OK手势精准抓握握拳竖大拇指数字小拇指半手指依次屈伸拇指对掌运动挥手动作暂停"
...
CJK_FONT_CANDIDATES = [
    ...
    "Songti SC",
    "STHeiti",
    # Linux / cross-platform CJK fonts
    "Noto Sans CJK SC",
    "Noto Sans SC",
    "Noto Serif CJK SC",
    "Source Han Sans SC",
    ...
    "AR PL UMing CN",
    "Arial Unicode MS",
    # Some CJK regional families still contain the simplified Chinese glyphs used here.
    "Noto Sans CJK JP",
    "Noto Sans CJK TC",
]
...
MPL_CJK_FONT_NAME: str | None | object = _UNCONFIGURED_FONT


def _font_supports_text(font_path: str, text: str = CJK_SAMPLE_TEXT) -> bool:
    """Return True when a font file contains all non-ASCII glyphs in text."""
    try:
        ...
        return False
    return all((ord(ch) < 128) or (ord(ch) in charmap) for ch in text)


def _unique_font_names(names: Iterable[str]) -> list[str]:
    seen: set[str] = set()
    result: list[str] = []
    for name in names:
...
def configure_matplotlib_fonts(preferred_font: str | None = None) -> str | None:
    """Configure Matplotlib to render Chinese pose names when a CJK font exists.

    preferred_font can be either a font family name such as "Microsoft YaHei" or
    a path to a .ttf/.otf file. When no Chinese-capable font is found, the code
    keeps Matplotlib's default font and later uses English action names.
    """
    global MPL_CJK_FONT_NAME
    if preferred_font is None and MPL_CJK_FONT_NAME is not _UNCONFIGURED_FONT:
        return MPL_CJK_FONT_NAME if isinstance(MPL_CJK_FONT_NAME, str) else None
    ...
    if preferred_font:
        preferred_path = Path(preferred_font).expanduser()
        if preferred_path.exists():
            try:
                font_manager.fontManager.addfont(str(preferred_path))
                candidate_names.append(font_manager.FontProperties(fname=str(preferred_path)).get_name())
            except Exception:
                # If addfont fails, keep going with normal font discovery.
                pass
        else:
            candidate_names.append(preferred_font)

    candidate_names.extend(CJK_FONT_CANDIDATES)
    candidate_names = _unique_font_names(candidate_names)

    fonts_by_name: dict[str, list[str]] = {}
    for font in font_manager.fontManager.ttflist:
        fonts_by_name.setdefault(font.name, []).append(font.fname)

    def apply_font(font_name: str) -> str:
    ...
    for font_name in candidate_names:
        for font_path in fonts_by_name.get(font_name, []):
            if _font_supports_text(font_path):
                MPL_CJK_FONT_NAME = apply_font(font_name)
                return MPL_CJK_FONT_NAME

    # Last resort: scan every installed Matplotlib-known font for CJK coverage.
    for font in font_manager.fontManager.ttflist:
        if _font_supports_text(font.fname):
            MPL_CJK_FONT_NAME = apply_font(font.name)
            return MPL_CJK_FONT_NAME

    mpl.rcParams["font.family"] = "sans-serif"
    mpl.rcParams["font.sans-serif"] = ["DejaVu Sans"]
    mpl.rcParams["axes.unicode_minus"] = False
    MPL_CJK_FONT_NAME = None
    return None


def cjk_font_available() -> bool:
    return isinstance(MPL_CJK_FONT_NAME, str)


def display_action_name(action_name: str) -> str:
    """Return a title-safe action name for the currently configured font."""
    if cjk_font_available():
        return action_name
    return ASCII_ACTION_NAMES.get(action_name, action_name)
...
def configure_axes(ax, action_name: str = "", paused: bool = False) -> None:
    ...
    safe_action_name = display_action_name(action_name)
    title = f"Dexterous Hand Kinematic Simulation | {safe_action_name}"
    if paused:
        title += " | paused"
    ax.set_title(title)
    try:
...
def init_3d_plot(preferred_font: str | None = None):
    """Initialize the Matplotlib 3D figure and axes."""
    plt = get_pyplot()
    font_name = configure_matplotlib_fonts(preferred_font)
    if font_name is None:
        print(
            "[font] No CJK-capable Matplotlib font was found; "
            "using English action names in the title. To show Chinese titles, "
            "install a CJK font such as Noto Sans CJK SC / SimHei / Microsoft YaHei, "
...
def run_animation(preferred_font: str | None = None) -> None:
    """Run the interactive Matplotlib animation."""
    plt = get_pyplot()
    fig, ax = init_3d_plot(preferred_font)
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional Chinese-capable font family name or path to a .ttf/.otf file, "
            "for example 'Microsoft YaHei', 'SimHei', or '/path/to/NotoSansCJKsc-Regular.otf'."
        ),
    )
    ...
    run_animation(preferred_font=args.font)
    return 0
